...
MEP元境 是一款 <b>开源</b> LLM应用开发平台，主攻<b>企业场景</b>， 已有大量行业头部组织及世界500强企业在使用。

“元境”是活字印刷术的发明人，活字印刷术为人类知识的传递起到了巨大的推动作用。我们希望“MEP元境”同样能够为智能应用的广泛落地提供有力支撑。欢迎大家一道参与。
...
2. **独具特色的[MEP workflow](https://dataelem.feishu.cn/wiki/R7HZwH5ZGiJUDrkHZXicA9pInif)**
   ...
   - 🖐️ **易用、符合直觉**：如成环、并行、批量运行操作，在同类产品中用户需借助专门组件实现，在MEP中只需完全按照直觉连接成“流程图”即可（画圈成环、并列即并行、多选即批量）。
...
> 除了MEP前后端，我们默认还会安装第三方组件ES、Milvus、Onlyoffice

下载MEP代码
```bash
# 如果系统中有git命令，可以直接下载元境代码
git clone https://github.com/dataelement/mep.git
# 进入安装目录
cd mep/docker

# 如果系统没有没有git命令，可以下载元境代码zip包
...
启动MEP
```bash
# 进入mep/docker或mep-main/docker目录，执行
...
- [📄MEP Blog](https://dataelem.feishu.cn/wiki/BiNowcaYWilewdksXQ5cZl3tnzy)
