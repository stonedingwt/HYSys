...
from mep.api.services.knowledge_imp import decide_vectorstores, extract_code_blocks
from mep.linsight.domain.schemas.inspiration_schema import SOPManagementSchema, SOPManagementUpdateSchema
from mep.linsight.domain.schemas.linsight_schema import SopRecordRead
...
from mep.common.errcode.linsight import (
...
from mep.common.errcode.server import (
...
from mep.linsight.domain.models.linsight_sop import LinsightSOP, LinsightSOPDao, LinsightSOPRecord
...
from mep_langchain.rag.init_retrievers import KeywordRetriever, BaselineVectorRetriever
from mep_langchain.retrievers import EnsembleRetriever
from mep_langchain.vectorstores import ElasticKeywordsSearch, Milvus
...
class SOPManageService:
    ...
    async def add_sop(sop_obj: SOPManagementSchema, user_id: int) -> UnifiedResponseModel | None:
        ...
        embeddings = await LLMService.get_mep_linsight_embedding(model_id=embed_info.id,
    ...
    async def update_sop(sop_obj: SOPManagementUpdateSchema,
        ...
        if sop_obj.content != existing_sop[0].content:
            ...
            embeddings = await LLMService.get_mep_linsight_embedding(invoke_user_id=sop_obj.user_id,
    ...
    async def search_sop(cls, invoke_user_id: int, query: str, k: int = 3) -> (List[Document], BaseErrorCode | None):
        ...
        try:
            ...
            if workbench_conf.embedding_model is None or not workbench_conf.embedding_model.id:
                ...
                try:
                    emb_model_id = workbench_conf.embedding_model.id
                    embeddings = await LLMService.get_mep_linsight_embedding(invoke_user_id=invoke_user_id,
                                                                                 model_id=int(emb_model_id))
