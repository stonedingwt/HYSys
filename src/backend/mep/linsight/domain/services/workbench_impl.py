...
from mep.api.services.knowledge_imp import decide_vectorstores, async_read_chunk_text
from mep.linsight.domain.services.sop_manage import SOPManageService
from mep.api.services.workstation import WorkStationService
from mep.linsight.domain.schemas.linsight_schema import LinsightQuestionSubmitSchema, DownloadFilesSchema, \
    SubmitFileSchema
...
from mep.common.errcode.http_error import UnAuthorizedError
from mep.common.errcode.linsight import LinsightToolInitError, LinsightMEPLLMError, LinsightGenerateSopError
from mep.common.schemas.telemetry.event_data_schema import NewMessageSessionEventData
...
from mep.core.cache.utils import save_file_to_folder, CACHE_DIR
...
from mep.linsight.domain.models.linsight_execute_task import LinsightExecuteTaskDao
from mep.linsight.domain.models.linsight_session_version import LinsightSessionVersionDao, SessionVersionStatusEnum, \
    LinsightSessionVersion
from mep.linsight.domain.models.linsight_sop import LinsightSOPRecord
from mep.database.models.session import MessageSessionDao, MessageSession
from mep.interface.embeddings.custom import FakeEmbedding
from mep.knowledge.domain.models.knowledge import KnowledgeRead, KnowledgeTypeEnum
...
from mep.tool.domain.services.tool import ToolServices
from mep.utils import util
from mep.utils.util import async_calculate_md5
from mep_langchain.linsight.const import ExecConfig
...
class LinsightWorkbenchImpl:
    ...
    class MEPLLMError(Exception):
        """MEP LLMRelated Errors"""

    @classmethod
    async def _get_llm(cls, invoke_user_id: int) -> (MEPLLM, Any):
        # Get and validate the workbench configuration
        workbench_conf = await cls._get_workbench_config()

        # BuatLLMInstances
        linsight_conf = settings.get_linsight_conf()
        llm = await LLMService.get_mep_linsight_llm(invoke_user_id=invoke_user_id,
    ...
    async def _get_workbench_config(cls):
        ...
        if not workbench_conf or not workbench_conf.task_model:
            raise cls.MEPLLMError("The task has been terminated, please contact the administrator to check the status of the Ideas task execution model")
    ...
    async def generate_sop(cls, linsight_session_version_id: str,
        ...
        except cls.MEPLLMError as e:
            logger.exception(f"MEP LLMError-free: session_version_id={linsight_session_version_id}, error={str(e)}")
            error_message = LinsightMEPLLMError(exception=e)
        except Exception as e:
    ...
    async def _create_linsight_agent(cls, session_version: LinsightSessionVersion,
                                     llm: MEPLLM, tools: List[BaseTool],
                                     workbench_conf):
        """BuatLinsightAgent"""
        from mep_langchain.linsight.agent import LinsightAgent
    ...
    async def _process_vector_storage(cls, invoke_user_id: int, texts: List[str], file_id: str,
        ...
        """Process vector storage"""
        # Buatembeddings
        embeddings = await LLMService.get_mep_linsight_embedding(model_id=workbench_conf.embedding_model.id,
                                                                     invoke_user_id=invoke_user_id)
    ...
    @classmethod
    async def _init_mep_code_tool(cls, config_tool_ids: List[int], file_dir: str, user_id: int) -> List[BaseTool]:
        """
        Code interpreter tool for special handling initialization Bi Lift
        """
        tools = []
        mep_code_tool = await GptsToolsDao.aget_tool_by_tool_key(tool_key='mep_code_interpreter')
        if not mep_code_tool or mep_code_tool.id not in config_tool_ids:
            return tools
        # Individual initialization code interpreter tool
        config_tool_ids.remove(mep_code_tool.id)
        code_config = json.loads(mep_code_tool.extra) if mep_code_tool.extra else {}
        ...
        mep_code_tool.extra = code_config

        tools = await ToolExecutor.init_by_tool_id(tool=mep_code_tool, app_id=ApplicationTypeEnum.LINSIGHT.value,
                                                   app_name=ApplicationTypeEnum.LINSIGHT.value,
                                                   app_type=ApplicationTypeEnum.LINSIGHT,
                                                   user_id=user_id)
        return [tools]
    ...
    async def init_linsight_config_tools(cls, session_version: LinsightSessionVersion,
                                         llm: MEPLLM, need_upload: bool = False, file_dir: str = None) -> List[
        BaseTool]:
        """
        ...
        if need_upload and file_dir:
            try:
                mep_code_tool = await cls._init_mep_code_tool(config_tool_ids, file_dir,
                                                                      user_id=session_version.user_id)
                tools.extend(mep_code_tool)
            except Exception as e:
