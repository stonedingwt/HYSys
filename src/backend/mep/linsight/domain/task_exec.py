...
from mep.linsight.domain.schemas.linsight_schema import UserInputEventSchema
from mep.common.services.config_service import settings
from mep.core.cache.utils import create_cache_folder_async, CACHE_DIR
from mep.core.external.http_client.http_client_manager import get_http_client
from mep.core.logger import trace_id_var
from mep.core.storage.minio.minio_manager import get_minio_storage
from mep.linsight.domain.models.linsight_execute_task import LinsightExecuteTaskDao, ExecuteTaskStatusEnum, \
...
from mep.linsight.domain import utils as linsight_execute_utils
from mep.linsight.domain.services.state_message_manager import LinsightStateMessageManager, MessageData, MessageEventType
...
from mep_langchain.linsight.const import TaskStatus, ExecConfig
from mep_langchain.linsight.event import NeedUserInput, GenerateSubTask, ExecStep, TaskStart, TaskEnd, BaseEvent
...
class LinsightWorkflowTask:
    ...
    async def _execute_workflow(self, session_model: LinsightSessionVersion):
        ...
        try:
            ...
            for one in tools:
                if one.name == "mep_code_interpreter":
                    one.close()
                    break
    ...
    async def _get_llm(self, invoke_user_id: int) -> BaseChatModel:
        ...
        try:
            ...
            return await LLMService.get_mep_linsight_llm(invoke_user_id=invoke_user_id,
