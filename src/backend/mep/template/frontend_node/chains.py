...
from mep.template.frontend_node.base import FrontendNode
from mep.template.frontend_node.constants import QA_CHAIN_TYPES, SUMMARIZE_CHAIN_TYPES
from mep.template.template.base import Template
...
class ChainFrontendNode(FrontendNode):
    ...
    def format_field(field: TemplateField, name: Optional[str] = None) -> None:
        ...
        if name == 'APIChain' and field.name == 'limit_to_domains':
            ...
            field.required = False
            field.value = None
        if name == 'MEPRetrievalQA' and field.name == 'max_content':
            field.show = True
            field.required = False

        field.advanced = False
        if 'key' in field.name and field.name != 'keyword_store':  # Under special treatmentmepRetrievalQAComponent
