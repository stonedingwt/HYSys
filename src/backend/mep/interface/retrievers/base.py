...
from mep.template.frontend_node.retrievers import RetrieverFrontendNode
from loguru import logger
from mep.utils.util import build_template_from_class, build_template_from_method
from mep_langchain import retrievers as mep_retrievers
    ...
    def type_to_loader_dict(self) -> Dict:
        if self.type_dict is None:
            ...
            self.type_dict.update({
                retriever_name:
                import_class(f'mep_langchain.retrievers.{retriever_name}')
                for retriever_name in mep_retrievers.__all__
            })
