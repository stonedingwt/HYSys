...
from mep.template.frontend_node.agents import AgentFrontendNode
from loguru import logger
from mep.utils.util import build_template_from_class, build_template_from_method
from mep_langchain import agents as mep_agents
...
class AgentCreator(LangChainTypeCreator):
    ...
    def type_to_loader_dict(self) -> Dict:
        if self.type_dict is None:
            ...
            mep = {
                chain_name: import_class(f'mep_langchain.agents.{chain_name}')
                for chain_name in mep_agents.__all__
            }
