...
from mep.interface.custom_lists import CUSTOM_NODES
from mep.interface.embeddings.custom import FakeEmbedding
from mep.interface.importing.utils import (eval_custom_component_code, get_function,
                                               import_by_type)
from mep.interface.initialize.llm import initialize_vertexai
from mep.interface.initialize.utils import (handle_format_kwargs, handle_node_type,
                                                handle_partial_variables, langchain_bug_openv1)
from mep.interface.initialize.vector_store import vecstore_initializer
...
from mep.interface.utils import load_file_into_dict
from mep.interface.wrappers.base import wrapper_creator
from mep.knowledge.domain.models.knowledge import KnowledgeDao
from mep.llm.domain import LLMService
from mep.utils import validate
from mep.utils.constants import NODE_ID_DICT, PRESET_QUESTION
from mep_langchain.vectorstores import VectorStoreFilterRetriever
...
# from mep_langchain.document_loaders.elem_unstrcutured_loader import ElemUnstructuredLoaderV0
...
def instantiate_embedding(class_object, params: Dict):
    ...
    try:
        ...
        elif class_object.__name__ == 'MEPEmbedding':
            params['app_id'] = "flow(will be deprecated)"
            params['app_name'] = "flow(will be deprecated)"
            params['app_type'] = ApplicationTypeEnum.SKILL
            params['user_id'] = params.get('user_id', 0)
        ...
        if node_type == 'MilvusWithPermissionCheck':
            ...
            for knowledge in knowledge_list:
                ...
                params['collection_embeddings'].append(
                    LLMService.get_mep_knowledge_embedding_sync(0, model_id=int(knowledge.model)))
