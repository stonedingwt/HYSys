...
async def _get_first_child_status(task_type_label: str, fallback: str = '销售订单下单') -> str:
    """从1002(任务状态)分类中，查找指定任务类型父项下sort_order最小的子状态值。"""
    try:
        ...
        async with get_async_db_session() as session:
            parent = (await session.exec(
                select(DictItem).where(
                    ...
                    DictItem.item_label == task_type_label,
                )
            )).first()
            if not parent:
                return fallback
        ...
        logger.warning('Failed to get first child status for %s', task_type_label)
        return fallback
...
async def _create_tasks_for_orders(
    ...
    agent_id = await _get_dict_value('task_type_agent', '跟单任务', fallback='')
    if not agent_id:
        logger.warning('No agent configured for task type sales_order_followup in data dict task_type_agent')
