...
from mep.api.v1.schemas import StreamData
from mep.core.database import get_sync_db_session
from mep.database.models.variable_value import Variable
...
def get_file_path_value(file_path):
    ...
    if not path.exists() or not str(path).startswith(user_cache_dir('mep', 'mep')):
        return ''
    return file_path
