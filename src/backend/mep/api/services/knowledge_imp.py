...
from mep.api.services.etl4lm_loader import Etl4lmLoader
from mep.api.services.libreoffice_converter import (
    convert_doc_to_docx,
    convert_ppt_to_pdf, convert_ppt_to_pptx,
)
from mep.api.services.md_from_pdf import is_pdf_damaged
from mep.api.services.patch_130 import (
    convert_file_to_md,
    combine_multiple_md_files_to_raw_texts,
)
from mep.api.v1.schemas import ExcelRule
from mep.common.constants.enums.telemetry import BaseTelemetryTypeEnum, ApplicationTypeEnum
from mep.common.constants.vectorstore_metadata import KNOWLEDGE_RAG_METADATA_SCHEMA
from mep.common.errcode import BaseErrorCode
from mep.common.errcode.knowledge import KnowledgeSimilarError, KnowledgeFileDeleteError, KnowledgeFileEmptyError, \
...
from mep.common.schemas.telemetry.event_data_schema import FileParseEventData
from mep.common.services import telemetry_service
from mep.common.services.config_service import settings
from mep.core.cache.redis_manager import get_redis_client_sync, get_redis_client
from mep.core.cache.utils import file_download
from mep.core.database import get_sync_db_session
from mep.core.logger import trace_id_var
from mep.core.storage.minio.minio_manager import get_minio_storage_sync, get_minio_storage
from mep.interface.embeddings.custom import FakeEmbedding
from mep.interface.importing.utils import import_vectorstore
from mep.interface.initialize.loading import instantiate_vectorstore
from mep.knowledge.domain.knowledge_rag import KnowledgeRag
from mep.knowledge.domain.models.knowledge import Knowledge, KnowledgeDao
from mep.knowledge.domain.models.knowledge_file import (
    KnowledgeFile,
    KnowledgeFileDao,
    KnowledgeFileStatus,
    ParseType,
...
from mep.knowledge.domain.schemas.knowledge_rag_schema import Metadata
from mep.llm.domain.services import LLMService
from mep.user.domain.models.user import UserDao
from mep.utils import md5_hash, util
from mep.utils.exceptions import EtlException, FileParseException
from mep_langchain.rag.extract_info import extract_title, async_extract_title
from mep_langchain.text_splitter import ElemCharacterTextSplitter
...
def decide_knowledge_llm(invoke_user_id: int) -> Any:
    ...
    # DapatkanllmObjects
    return LLMService.get_mep_llm_sync(
...
async def async_decide_knowledge_llm(invoke_user_id: int) -> Any:
    ...
    # DapatkanllmObjects
    return await LLMService.get_mep_llm(
        model_id=knowledge_llm.extract_title_model_id,
...
def text_knowledge(
    ...
    embeddings = LLMService.get_mep_knowledge_embedding_sync(model_id=int(db_knowledge.model),
...
def QA_save_knowledge(db_knowledge: Knowledge, QA: QAKnowledge):
    ...
    try:
        embeddings = LLMService.get_mep_knowledge_embedding_sync(invoke_user_id=QA.user_id,
