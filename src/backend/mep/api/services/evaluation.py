...
import pandas as pd
try:
    from mep_ragas import evaluate
    from mep_ragas.llms.langchain import LangchainLLM
    from mep_ragas.metrics import AnswerCorrectnessMEP
except (ImportError, ModuleNotFoundError):
    evaluate = None
    LangchainLLM = None
    AnswerCorrectnessMEP = None
...
from mep.api.services.flow import FlowService
...
from mep.database.models.evaluation import (Evaluation, EvaluationDao, ExecType, EvaluationTaskStatus)
from mep.database.models.flow import FlowDao
from mep.database.models.flow_version import FlowVersionDao, FlowVersion
...
async def add_evaluation_task(evaluation_id: int):
    ...
    try:
        ...
        answer_correctness_mep = AnswerCorrectnessMEP(llm=llm, human_prompt=evaluation.prompt)
        score = await asyncio.to_thread(evaluate, dataset, [answer_correctness_mep])
        df = score.to_pandas()
