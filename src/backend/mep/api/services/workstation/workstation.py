...
from mep.api.v1.schema.chat_schema import UseKnowledgeBaseParam
from mep.api.v1.schemas import KnowledgeFileOne, KnowledgeFileProcess, WorkstationConfig
from mep.common.dependencies.user_deps import UserPayload
from mep.common.errcode.server import EmbeddingModelStatusError
from mep.common.models.config import Config, ConfigDao, ConfigKeyEnum
from mep.core.vectorstore.multi_retriever import MultiRetriever
from mep.database.constants import MessageCategory
...
from mep.tool.domain.langchain.knowledge import KnowledgeRetrieverTool
...
class WorkStationService(BaseService):
    ...
    def parse_config(cls, config: Any) -> Optional[WorkstationConfig]:
        if config:
            ...
            # Compatible with older webSearch config (only if tool is not explicitly set)
            if ret.webSearch and ret.webSearch.params is None and not ret.webSearch.tool:
    ...
    def uploadPersonalKnowledge(
        ...
        try:
            _ = LLMService.get_mep_knowledge_embedding(login_user.user_id, int(knowledge.model))
