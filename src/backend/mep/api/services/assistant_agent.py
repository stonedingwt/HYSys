...
from mep.api.services.assistant_base import AssistantUtils
from mep.api.utils import build_flow_no_yield
from mep.api.v1.schemas import InputRequest
from mep.common.constants.enums.telemetry import ApplicationTypeEnum
from mep.common.errcode.assistant import AssistantModelEmptyError, AssistantModelNotConfigError, \
    AssistantAutoLLMError
from mep.database.models.assistant import Assistant, AssistantLink, AssistantLinkDao
from mep.database.models.flow import FlowDao, FlowStatus
from mep.llm.domain.services import LLMService
from mep.tool.domain.services.executor import ToolExecutor
from mep_langchain.gpts.assistant import ConfigurableAssistant
from mep_langchain.gpts.auto_optimization import (generate_breif_description,
                                                      generate_opening_dialog,
                                                      optimize_assistant_prompt)
from mep_langchain.gpts.auto_tool_selected import ToolInfo, ToolSelector
from mep_langchain.gpts.prompts import ASSISTANT_PROMPT_OPT
...
class AssistantAgent(AssistantUtils):
    ...
    async def init_llm(self):
        ...
        # Inisialisasillm
        self.llm = await LLMService.get_mep_llm(model_id=default_llm.model_id,
    ...
    async def init_auto_update_llm(self):
        ...
        self.llm = await LLMService.get_mep_llm(model_id=assistant_llm.auto_llm.model_id,
                                                    temperature=self.assistant.temperature,
    ...
    async def record_chat_history(self, message: List[Any]):
        ...
        if not os.getenv('MEP_RECORD_HISTORY'):
            return
        try:
