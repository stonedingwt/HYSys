...
from mep.api.services.knowledge_imp import decide_vectorstores
from mep.linsight.domain.models.linsight_session_version import LinsightSessionVersionDao
...
class SearchKnowledgeBase(BaseTool):
    ...
    async def search_linsight_file(self, query: str, file_id: str, limit: int) -> str:
        ...
        embeddings = await LLMService.get_mep_linsight_embedding(session_info.user_id,
    ...
    async def search_knowledge(self, query: str, knowledge_id: int, limit: int) -> str:
        ...
        embeddings = await LLMService.get_mep_knowledge_embedding(0, model_id=int(knowledge_info.model))
