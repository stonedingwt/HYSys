...
from mep.api.services.knowledge_imp import (decide_vectorstores, delete_es, delete_vector,
                                                text_knowledge)
from mep.api.v1.schemas import (ChunkInput, KnowledgeFileOne, KnowledgeFileProcess,
                                    resp_200, resp_500, ExcelRule)
from mep.common.constants.enums.telemetry import BaseTelemetryTypeEnum
from mep.common.errcode.http_error import ServerError
from mep.common.services import telemetry_service
from mep.common.services.config_service import settings
from mep.core.cache.utils import file_download, save_download_file, async_file_download
...
from mep.knowledge.domain.models.knowledge import (KnowledgeCreate, KnowledgeDao, KnowledgeTypeEnum,
                                                       KnowledgeUpdate)
from mep.knowledge.domain.models.knowledge_file import (QAKnoweldgeDao, QAKnowledgeUpsert)
from mep.open_endpoints.domain.schemas.filelib import APIAddQAParam, APIAppendQAParam, QueryQAParam
from mep.open_endpoints.domain.utils import get_default_operator, get_default_operator_async
from mep.utils.util import sync_func_to_async
...
async def upload_file(
    ...
    if file:
        ...
        # Cache Local
        file_path = await sync_func_to_async(save_download_file)(save_download_file, file.file, 'mep', file_name)
    else:
...
async def post_chunks(request: Request,
    ...
    if not file_name:
        return resp_500(message='file name must be not empty')
    file_path = await sync_func_to_async(save_download_file)(file.file, 'mep', file_name)
...
async def post_string_chunks(request: Request, document: ChunkInput):
    ...
    file_path = await sync_func_to_async(save_download_file)(content_bytes, 'mep', file_name)
