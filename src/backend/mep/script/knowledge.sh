...
if [ "$run_mode" = "convert_all" ]; then
    ...
    python mep/script/knowledge_data_convert.py --mode convert_all
elif [ "$run_mode" = "convert_one" ]; then
    echo "Converting one knowledge : $knowledge_id"
    python mep/script/knowledge_data_convert.py --mode convert_one --id ${knowledge_id}
elif [ "$run_mode" = "scan_all" ]; then
    echo "Scanning all knowledge files..."
    python mep/script/knowledge_data_fix.py --mode scan_all
elif [ "$run_mode" = "scan_one" ]; then
    echo "Scanning single knowledge : $knowledge_id"
    python mep/script/knowledge_data_fix.py --mode scan_one --id ${knowledge_id}
elif [ "$run_mode" = "fix_all" ]; then
    echo "Fixing all knowledge files..."
    python mep/script/knowledge_data_fix.py --mode fix_all
elif [ "$run_mode" = "fix_one" ]; then
    echo "Fixing single knowledge : $knowledge_id"
    python mep/script/knowledge_data_fix.py --mode fix_one --id ${knowledge_id}
else
