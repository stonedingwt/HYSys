...
from mep.common.constants.enums.telemetry import ApplicationTypeEnum
from mep.common.dependencies.user_deps import UserPayload
from mep.common.errcode.http_error import NotFoundError, ServerError
from mep.common.errcode.llm import ServerExistError, ModelNameRepeatError, ServerAddError, ServerAddAllError
from mep.common.errcode.server import NoAsrModelConfigError, AsrModelConfigDeletedError, NoTtsModelConfigError, \
    TtsModelConfigDeletedError
from mep.common.models.config import ConfigDao, ConfigKeyEnum, Config
...
from mep.knowledge.domain.models.knowledge import KnowledgeDao, KnowledgeTypeEnum
from mep.knowledge.domain.models.knowledge import KnowledgeState
from mep.llm.domain.const import LLMModelType
from mep.llm.domain.models import LLMDao, LLMServer, LLMModel
from mep.llm.domain.schemas import LLMServerInfo, LLMModelInfo, KnowledgeLLMConfig, AssistantLLMConfig, \
    EvaluationLLMConfig, AssistantLLMItem, LLMServerCreateReq, WorkbenchModelConfig, WSModel
from mep.utils import generate_uuid, md5_hash
from mep.utils.mask_data import JsonFieldMasker
from ..llm import MEPASR, MEPLLM, MEPTTS, MEPEmbedding
from ..llm.rerank import MEPRerank


class LLMService:

    @classmethod
    def get_qwen_server_config(cls) -> Optional[Dict]:
        """Get the config of the first Qwen-type LLM server (for web search, etc.)."""
        servers = LLMDao.get_all_server()
        for s in servers:
            if s.type == 'qwen' and s.config:
                return s.config
        return None
        ...
        for one in ret.models:
            try:
                if one.model_type == LLMModelType.LLM.value:
                    await cls.get_mep_llm(model_id=one.id, ignore_online=True, **common_params)
                elif one.model_type == LLMModelType.EMBEDDING.value:
                    await cls.get_mep_embedding(model_id=one.id, ignore_online=True, **common_params)
                elif one.model_type == LLMModelType.ASR.value:
                    await cls.get_mep_asr(model_id=one.id, ignore_online=True, **common_params)
                elif one.model_type == LLMModelType.TTS.value:
                    await cls.get_mep_tts(model_id=one.id, ignore_online=True, **common_params)
    ...
    async def test_model_status(cls, model: LLMModel | LLMModelInfo, login_user: UserPayload):
        ...
        try:
            if model.model_type == LLMModelType.LLM.value:
                mep_model = await cls.get_mep_llm(model_id=model.id, ignore_online=True, **common_params)
                await mep_model.ainvoke('hello')
            elif model.model_type == LLMModelType.EMBEDDING.value:
                mep_embed = await cls.get_mep_embedding(model_id=model.id, ignore_online=True, **common_params)
                await mep_embed.aembed_query('hello')
            elif model.model_type == LLMModelType.TTS.value:
                mep_tts = await cls.get_mep_tts(model_id=model.id, ignore_online=True, **common_params)
                await mep_tts.ainvoke('hello')
                ...
                with open(example_file_path, 'rb') as f:
                    mep_asr = await cls.get_mep_asr(model_id=model.id, ignore_online=True, **common_params)
                    await mep_asr.ainvoke(f)
            elif model.model_type == LLMModelType.RERANK.value:
                mep_rerank = await cls.get_mep_rerank(model_id=model.id, ignore_online=True, **common_params)
                await mep_rerank.acompress_documents(documents=[Document(page_content="hello world")],
                                                         query="hello")
        except Exception as e:
    ...
    def get_knowledge_source_llm(cls, invoke_user_id: int) -> Optional[BaseChatModel]:
        ...
        if not knowledge_llm.source_model_id:
            return None
        return cls.get_mep_llm_sync(model_id=knowledge_llm.source_model_id,
    ...
    async def get_knowledge_source_llm_async(cls, invoke_user_id: int) -> Optional[BaseChatModel]:
        ...
        if not knowledge_llm.source_model_id:
            return None
        return await cls.get_mep_llm(model_id=knowledge_llm.source_model_id,
                                        app_id=ApplicationTypeEnum.RAG_TRACEABILITY.value,
                                        app_name=ApplicationTypeEnum.RAG_TRACEABILITY.value,
                                        app_type=ApplicationTypeEnum.RAG_TRACEABILITY,
                                        user_id=invoke_user_id)
    ...
    def get_knowledge_similar_llm(cls, invoke_user_id: int) -> Optional[BaseChatModel]:
        ...
        if not knowledge_llm.qa_similar_model_id:
            return None
        return cls.get_mep_llm_sync(model_id=knowledge_llm.qa_similar_model_id,
    ...
    def get_knowledge_default_embedding(cls, invoke_user_id: int) -> Optional[Embeddings]:
        ...
        if not knowledge_llm.embedding_model_id:
            return None
        return cls.get_mep_knowledge_embedding_sync(model_id=knowledge_llm.embedding_model_id,
    ...
    async def get_evaluation_llm_object(cls, invoke_user_id: int) -> BaseChatModel:
        ...
        return await cls.get_mep_llm(model_id=evaluation_llm.model_id,
    ...
    @classmethod
    async def get_mep_llm(cls, **kwargs) -> BaseChatModel:
        """ Initialize LiftedllmConversation Model """
        return await MEPLLM.get_mep_llm(**kwargs)

    @classmethod
    def get_mep_llm_sync(cls, **kwargs) -> BaseChatModel:
        """ Initialize LiftedllmConversation Model """
        return MEPLLM(**kwargs)

    @classmethod
    async def get_mep_linsight_llm(cls, invoke_user_id: int, **kwargs) -> BaseChatModel:
        return await MEPLLM.get_mep_llm(app_id=ApplicationTypeEnum.LINSIGHT.value,
    ...
    @classmethod
    async def get_mep_rerank(cls, **kwargs) -> BaseDocumentCompressor:
        return await MEPRerank.get_mep_rerank(**kwargs)

    @classmethod
    def get_mep_rerank_sync(cls, **kwargs) -> BaseDocumentCompressor:
        return MEPRerank(**kwargs)

    @classmethod
    async def get_mep_embedding(cls, **kwargs) -> Embeddings:
        """ Initialize LiftedembeddingModels """
        return await MEPEmbedding.get_mep_embedding(**kwargs)

    @classmethod
    def get_mep_embedding_sync(cls, **kwargs) -> Embeddings:
        """ Initialize LiftedembeddingModels """
        return MEPEmbedding(**kwargs)

    @classmethod
    async def get_mep_daily_embedding(cls, invoke_user_id: int, model_id: int) -> Embeddings:
        """ Get dailyembeddingModels """
        return await cls.get_mep_embedding(model_id=model_id,
    ...
    @classmethod
    async def get_mep_linsight_embedding(cls, invoke_user_id: int, model_id: int) -> Embeddings:
    ...
    @classmethod
    async def get_mep_knowledge_embedding(cls, invoke_user_id: int, model_id: int) -> Embeddings:
        """ Get Knowledge Base DefaultsembeddingModels """
        return await cls.get_mep_embedding(model_id=model_id,
                                               app_id=ApplicationTypeEnum.KNOWLEDGE_BASE.value,
                                               app_name=ApplicationTypeEnum.KNOWLEDGE_BASE.value,
    ...
    @classmethod
    def get_mep_knowledge_embedding_sync(cls, invoke_user_id: int, model_id: int) -> Embeddings:
        """ Get Knowledge Base DefaultsembeddingModels """
        return cls.get_mep_embedding_sync(model_id=model_id,
                                              app_id=ApplicationTypeEnum.KNOWLEDGE_BASE.value,
                                              app_name=ApplicationTypeEnum.KNOWLEDGE_BASE.value,
                                              app_type=ApplicationTypeEnum.KNOWLEDGE_BASE,
                                              user_id=invoke_user_id)
    ...
    @classmethod
    async def get_mep_asr(cls, **kwargs) -> MEPASR:
        """ Initialize LiftedasrModels """
        return await MEPASR.get_mep_asr(**kwargs)

    @classmethod
    async def get_mep_tts(cls, **kwargs) -> MEPTTS:
        """ Initialize LiftedttsModels """
        return await MEPTTS.get_mep_tts(**kwargs)
    ...
    async def update_workbench_llm(cls, invoke_user_id: int, config_obj: WorkbenchModelConfig,
        ...
        if config_obj.embedding_model:
            ...
            if (config_obj.embedding_model.id and config_old_obj.embedding_model is None or
                ...
                embeddings = await cls.get_mep_embedding(model_id=config_obj.embedding_model.id,
                                                             app_id=ApplicationTypeEnum.LINSIGHT.value,
                                                             app_name=ApplicationTypeEnum.LINSIGHT.value,
                                                             app_type=ApplicationTypeEnum.LINSIGHT,
                                                             user_id=invoke_user_id)
                ...
                from mep.linsight.domain.services.sop_manage import SOPManageService
    ...
    async def invoke_workbench_asr(cls, login_user: UserPayload, file: UploadFile) -> str:
        ...
        asr_client = await cls.get_mep_asr(model_id=int(workbench_llm.asr_model.id),
    ...
    async def invoke_workbench_tts(cls, login_user: UserPayload, text: str) -> str:
        ...
        tts_client = await cls.get_mep_tts(model_id=int(workbench_llm.tts_model.id),
