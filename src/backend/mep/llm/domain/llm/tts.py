...
from mep.common.errcode.server import NoTtsModelConfigError, TtsModelConfigDeletedError, \
    TtsModelTypeError, TtsProviderDeletedError, TtsModelOfflineError
from mep.core.ai import BaseTTSClient, OpenAITTSClient, \
...
class MEPTTS(MEPBase):
    tts: BaseTTSClient = Field(..., description="ttsInstances")

    @classmethod
    async def get_mep_tts(cls, **kwargs) -> Self:
