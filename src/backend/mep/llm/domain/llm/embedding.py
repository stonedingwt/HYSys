...
from mep.core.ai import OllamaEmbeddings, OpenAIEmbeddings, AzureOpenAIEmbeddings, DashScopeEmbeddings
from mep.llm.domain.const import LLMServerType, LLMModelType
from .base import MEPBase
from ..models import LLMModel, LLMServer
from ..utils import wrapper_mep_model_limit_check
...
class MEPEmbedding(MEPBase, Embeddings):
    ...
    @classmethod
    async def get_mep_embedding(cls, **kwargs) -> Self:
    ...
    def _init_client(self, model_info, server_info, **kwargs):
        ...
        logger.debug(
            f'init_mep_embedding: server_id: {server_info.id}, model_id: {model_info.id}')
        ...
        try:
            ...
            logger.exception('init_mep_embedding error')
            raise Exception(f'Inisialisasimep embeddingComponent failed, please check the configuration or contact the administrator.Error message:{e}')
