...
from mep.common.errcode.server import NoLlmModelConfigError, LlmModelConfigDeletedError, LlmProviderDeletedError, \
    LlmModelTypeError, LlmModelOfflineError, InitLlmError
from mep.core.ai import ChatOllama, ChatOpenAI, ChatOpenAICompatible, \
    AzureChatOpenAI, ChatZhipuAI, MiniMaxChat, ChatAnthropic, MoonshotChat
from mep.core.ai.llm.custom_chat_deepseek import CustomChatDeepSeek
from mep.core.ai.llm.custom_chat_tongyi import CustomChatTongYi
from mep.llm.domain.const import LLMModelType, LLMServerType
from mep.llm.domain.models import LLMServer, LLMModel
from .base import MEPBase
from ..utils import wrapper_mep_model_limit_check, wrapper_mep_model_limit_check_async, \
    wrapper_mep_model_generator, wrapper_mep_model_generator_async
...
class MEPLLM(MEPBase, BaseChatModel):
    """
    ...
    @classmethod
    async def get_mep_llm(cls, **kwargs) -> Self:
    ...
    def _init_client(self, model_info: LLMModel, server_info: LLMServer, **kwargs):
        ...
        logger.debug(f'init_mep_llm: server_id: {server_info.id}, model_id: {model_info.id}')
        ...
        try:
            ...
            logger.exception('init mep llm error')
    ...
    @wrapper_mep_model_generator_async
    async def _astream(
            self,
            messages: list[BaseMessage],
            stop: Optional[list[str]] = None,
