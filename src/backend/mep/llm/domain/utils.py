...
from mep.common.constants.enums.telemetry import StatusEnum, BaseTelemetryTypeEnum
from mep.common.schemas.telemetry.event_data_schema import ModelInvokeEventData
...
from mep.llm.domain.const import LLMModelStatus


async def mep_model_limit_check(self: 'MEPBase'):
...
def sync_mep_model_limit_check(self: 'MEPBase'):
    now = datetime.now().strftime("%Y-%m-%d")
    if self.server_info.limit_flag:
        # Number of calls checked
        cache_key = f"model_limit:{now}:{self.server_info.id}"
...
def upload_telemetry_log(self: 'MEPBase', start_time: float, end_time: float, first_token_cost_time: int,
...
def wrapper_mep_model_limit_check(func):
    """
    ...
    def wrapper(*args, **kwargs):
        self = args[0]
        sync_mep_model_limit_check(self)
...
def wrapper_mep_model_limit_check_async(func):
    """
    Number of calls to check the decorator
    """
    ...
    async def wrapper(*args, **kwargs):
        self = args[0]
        await mep_model_limit_check(self)
        status = LLMModelStatus.NORMAL.value
        remark = ""
        start_time = time.time()
        telemetry_status = StatusEnum.SUCCESS
...
def wrapper_mep_model_generator(func):
    """
...
def wrapper_mep_model_generator_async(func):
    """
