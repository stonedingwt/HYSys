...
class SystemLoginMethod(BaseModel):
    ...
    mep_pro: bool = Field(default=False, description='Whether it is a commercial version, Verify Configuredlicense')
    ...
    sso_type: Optional[str] = Field(default='none', description='Third-party SSO type: none, dingtalk, wecom, feishu, aad')
    gateway_login: bool = Field(default=False, description='Whether third-party gateway login is enabled')
    dingtalk: Optional[dict] = Field(default=None, description='DingTalk SSO config')
    wecom: Optional[dict] = Field(default=None, description='WeCom SSO config')
    feishu: Optional[dict] = Field(default=None, description='Feishu SSO config')
    aad: Optional[dict] = Field(default=None, description='Azure AD SSO config')
...
class MinioConf(BaseModel):
    ...
    public_bucket: Optional[str] = Field(default="mep",
...
class CeleryConf(BaseModel):
    ...
    def validate(self):
        if not self.task_routers:
            self.task_routers = {
                "mep.worker.knowledge.*": {"queue": "knowledge_celery"},  # Knowledge Base Related Tasks
                "mep.worker.workflow.*": {"queue": "workflow_celery"},  # Workflow Execution Related Tasks
                "mep.worker.scheduled_task.*": {"queue": "workflow_celery"},  # Scheduled Task Related
            }
        if 'telemetry_mid_user_increment' not in self.beat_schedule:
            self.beat_schedule['telemetry_mid_user_increment'] = {
                'task': 'mep.worker.telemetry.mid_table.sync_mid_user_increment',
        ...
        if 'telemetry_mid_knowledge_increment' not in self.beat_schedule:
            self.beat_schedule['telemetry_mid_knowledge_increment'] = {
                'task': 'mep.worker.telemetry.mid_table.sync_mid_knowledge_increment',
                'schedule': crontab('*/30 0 * * *'),  # 00:30 exec every day
            }
        if 'telemetry_sync_mid_app_increment' not in self.beat_schedule:
            self.beat_schedule['telemetry_sync_mid_app_increment'] = {
                'task': 'mep.worker.telemetry.mid_table.sync_mid_app_increment',
                'schedule': crontab('*/30 0 * * *'),  # 00:30 exec every day
            }
        if 'telemetry_sync_mid_user_interact_dtl' not in self.beat_schedule:
            self.beat_schedule['telemetry_sync_mid_user_interact_dtl'] = {
                'task': 'mep.worker.telemetry.mid_table.sync_mid_user_interact_dtl',
                'schedule': crontab('*/30 0 * * *'),  # 00:30 exec every day
            }

        if 'scheduled_task_checker' not in self.beat_schedule:
            self.beat_schedule['scheduled_task_checker'] = {
                'task': 'mep.worker.scheduled_task.tasks.check_scheduled_tasks',
...
class CookieConf(BaseModel):
    ...
    jwt_iss: str = Field(default='mep', description="JwtTokenIssuer of")
...
class Settings(BaseModel):
    ...
    def set_database_url(cls, value):
        if not value:
            logger.debug('No database_url provided, trying mep_DATABASE_URL env variable')
            if mep_database_url := os.getenv('mep_DATABASE_URL'):
                value = mep_database_url
            else:
