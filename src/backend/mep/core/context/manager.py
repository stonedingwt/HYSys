...
from mep.core.config.settings import Settings
from mep.core.context.base import (
    ContextRegistry,
    BaseContextManager,
    ContextState,
    ContextError
...
class ApplicationContextManager:
    ...
    def _register_default_contexts(self, config: Settings) -> None:
        ...
        try:
            ...
            from mep.core.database.manager import DatabaseManager
            self.register_context(DatabaseManager(database_url=config.database_url))

            from mep.core.cache.redis_manager import RedisManager
            self.register_context(RedisManager(redis_url=config.redis_url))

            from mep.core.storage.minio.minio_manager import MinioManager
            self.register_context(MinioManager(minio_config=config.object_storage.minio))

            from mep.core.search.elasticsearch.manager import EsConnManager, statistics_es_name
            ...
            from mep.core.external.http_client.http_client_manager import HttpClientManager
            self.register_context(HttpClientManager())

            from mep.core.prompts.manager import PromptManager
