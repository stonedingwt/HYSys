...
from mep.core.storage.minio.minio_manager import get_minio_storage, get_minio_storage_sync

CACHE: Dict[str, Any] = {}

CACHE_DIR = user_cache_dir('mep', 'mep')
...
def file_download(file_path: str):
    ...
    if _is_valid_url(file_path):
        ...
        file_path = save_download_file(file_content, 'mep', filename)
        return file_path, filename

    # <g id="Bold">Medical Treatment:</g> MinIO Relative path (In / Starts with a signature parameter)
    # For Input: /mep/original/82324.docx?X-Amz-Algorithm=...
    # No in this case host, cannot be accessed _is_valid_url Branch
        ...
        try:
            ...
            url_obj = urlparse(file_path)
            # path similar to /mep/original/82324.docx
...
async def async_file_download(file_path: str):
    ...
    # For Input: /mep/original/82324.docx?X-Amz-Algorithm=...
        ...
        try:
            ...
            url_obj = urlparse(file_path)
            # path turned into /mep/original/82324.docx, remove the opening / and split the first one /
            path_parts = url_obj.path.lstrip("/").split('/', 1)
