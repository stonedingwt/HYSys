...
from mep.workflow.callback.event import StreamMsgOverData
from mep.workflow.callback.llm_callback import LLMNodeCallbackHandler
from mep.workflow.nodes.base import BaseNode
from mep.workflow.nodes.prompt_template import PromptTemplateParser
...
class AgentNode(BaseNode):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._llm = LLMService.get_mep_llm_sync(model_id=self.node_params['model_id'],
    ...
    def _init_agent(self, system_prompt: str):
        ...
        node_model_id = self.node_params['model_id']
        if isinstance(node_model_id, str) and node_model_id.isdigit():
            node_model_id = int(node_model_id)
        default_llm = [
            one for one in assistant_llm.llm_list if one.model_id == node_model_id
        ]
        if not default_llm:
