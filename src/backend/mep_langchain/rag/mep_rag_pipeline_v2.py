...
from mep_langchain.rag.init_retrievers import (
    BaselineVectorRetriever,
    KeywordRetriever,
    MixRetriever,
    SmallerChunksVectorRetriever,
)
from mep_langchain.rag.scoring.ragas_score import RagScore
from mep_langchain.rag.extract_info import extract_title
from mep_langchain.rag.utils import import_by_type, import_class


class MEPRagPipeline:
    ...
    def __init__(self, yaml_path) -> None:
        ...
        if self.params['elasticsearch'].get('extract_key_by_llm', False):
            extract_key_prompt = import_class(f'mep_langchain.rag.prompts.EXTRACT_KEY_PROMPT')
    ...
    def retrieval_and_rerank(self, question, collection_name, max_content=100000):
        ...
        if self.params['post_retrieval']['with_rank'] and len(docs):
            if not hasattr(self, 'ranker'):
                ...
                rerank_object = import_class(f'mep_langchain.rag.rerank.{rerank_type}')
    ...
    def question_answering(self):
        ...
        if 'prompt_type' in self.params['generate']:
            ...
            prompt = import_class(f'mep_langchain.rag.prompts.{prompt_type}')
        else:
            prompt = None
    ...
    def score(self):
        ...
        if metric_params['type'] == 'mep-ragas':
            score_params = {
...
if __name__ == '__main__':
    ...
    parser.add_argument('--params', type=str, default='config/test/baseline_s2b.yaml', help='mep rag params')
    # 解析参数
    args = parser.parse_args()

    rag = MEPRagPipeline(args.params)
