...
from .mep_rag_tool import MEPRAGTool
...
class MEPRetrievalQA(Chain):
    ...
    mep_rag_tool: MEPRAGTool = Field(default_factory=MEPRAGTool,
    ...
    def from_llm(
        ...
        **kwargs: Any,
        ...
        mep_rag_tool = MEPRAGTool(vector_store=vector_store,
    ...
    def _call(
        ...
        if self.return_source_documents:
            answer, docs = self.mep_rag_tool.run(
                question,
            ...
            answer = self.mep_rag_tool.run(question, return_only_outputs=True)
    ...
    async def _acall(
        ...
        if self.return_source_documents:
            answer, docs = await self.mep_rag_tool.arun(question, return_only_outputs=False)
            return {self.output_key: answer, 'source_documents': docs}
        else:
            answer = await self.mep_rag_tool.arun(question, return_only_outputs=True)
            return {self.output_key: answer}
