...
try:
    ...
    from mep_ragas.metrics import AnswerCorrectness, AnswerCorrectnessMEP, AnswerRecallMEP
except (ImportError, ModuleNotFoundError):
    evaluate = None
    AnswerCorrectness = None
    AnswerCorrectnessMEP = None
    AnswerRecallMEP = None
...
class RagScore:
    ...
    def ragas_answer_correctness_mep(self, dataset: Dataset) -> pd.DataFrame:
        answer_correctness = AnswerCorrectnessMEP(batch_size=self.batch_size)
        result = evaluate(
            dataset=dataset,
    ...
    def ragas_answer_recall_mep(self, dataset: Dataset) -> pd.DataFrame:
        answer_recall =AnswerRecallMEP(batch_size=self.batch_size, 
    ...
    def score(self) -> Tuple[pd.DataFrame, Optional[pd.DataFrame]]:
        ...
        for metric_name in self.metrics:
            ...
            if metric_name =='answer_recall_mep':
                if self.gt_split_column:
...
if __name__ == '__main__':
    params = {
        'excel_path': '/home/gulixin/workspace/llm/mep/src/mep-langchain/experimental/rag/data/test.xlsx',
        'save_path': '/home/gulixin/workspace/llm/mep/src/mep-langchain/experimental/rag/data',
        'question_column': '问题',
        'gt_column': 'GT',
        'answer_column': 'rag_answer',
        'query_type_column': '问题类型',
        # 'metrics': ['answer_correctness_mep'],
        'metrics': ['answer_recall_mep'],
        'batch_size': 10,
