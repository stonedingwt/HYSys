...
from mep_langchain.rag.init_retrievers import (BaselineVectorRetriever, KeywordRetriever,
        ...
        if keyword_store:
            ...
            if self.params['elasticsearch'].get('extract_key_by_llm', False):
                extract_key_prompt = import_class(
                    'mep_langchain.rag.prompts.EXTRACT_KEY_PROMPT')
...
if __name__ == '__main__':
    # rag_tool = MEPRAGTool(collection_name='rag_finance_report_0_test')
    ...
    tool = MEPRAGTool.get_rag_tool(name='rag_knowledge_retrieve',
