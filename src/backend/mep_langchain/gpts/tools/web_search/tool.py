...
class QwenSearch(SearchTool):
    """Uses DashScope-compatible API with built-in web search to retrieve results."""

    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        from mep.common.services.config_service import settings
        from mep.llm.domain.services.llm import LLMService
        server_config = LLMService.get_qwen_server_config()
        self.api_key = server_config.get('openai_api_key', '') if server_config else ''
    ...
    def _invoke(self, query: str, **kwargs) -> (str, list):
        ...
        payload = {
            'model': 'qwen-max',
            'messages': [
                {'role': 'system', 'content': '你是一个搜索助手。请根据用户的搜索词进行联网检索，返回相关的搜索结果。'},
                {'role': 'user', 'content': query}
            ],
            'enable_search': True,
        }
        result = self._requests(self.base_url, method='POST', json=payload, headers=headers)

        web_list = []
        search_results = result.get('choices', [{}])[0].get('message', {}).get('search_results', [])
        if search_results:
            for item in search_results:
                web_list.append({
                    ...
                    'thumbnail': None,
                    'site_name': self.get_host_from_url(item.get('url', '') or item.get('link', '')),
                })
