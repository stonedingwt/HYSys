...
from mep_langchain.utils.openapi import convert_openapi_field_value
    ...
    def parse_args_schema(self):
        ...
        for one in params:
            ...
            if field_type in ['number', 'integer', 'string', 'boolean']:
                ...
                schema_props = one['schema'].get('properties', {})
                for param in schema_props.keys():
                    field_type = schema_props[param]['type']
                    if field_type in ['number', 'integer', 'string', 'boolean']:
                        object_field_info = {
                            "type": field_type,
                            "description": one['description']
