...
import {
  ...
  ArrowUpDown, ArrowUp, ArrowDown, Download, Filter, FilterX,
} from 'lucide-react';
...
interface ColDef { key: string; label: string; width?: number; type?: 'text' | 'number'; }
...
export default function WsSalesOrder() {
  ...
  const totalWidth = HEADER_COLS.reduce((sum, c) => sum + (c.width || 120), 0) + 140;
  ...
  return (
    <div className="h-full overflow-hidden bg-[#f4f5f8] dark:bg-[#111] flex flex-col p-5">
      ...
      {/* Table */}
      <div ref={tableRef} className="flex-1 overflow-auto rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1B1B1B]">
        <table className="text-sm" style={{ minWidth: `${totalWidth}px` }}>
          <thead className="sticky top-0 z-10">
            <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#222]">
              <th className="w-[40px] min-w-[40px] px-2 py-2.5"></th>
              {HEADER_COLS.map(c => (
              ...
              <th className="text-center px-3 py-2.5 font-medium text-gray-600 dark:text-gray-300 w-[100px] min-w-[100px]">操作</th>
            </tr>
            {showFilters && (
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={HEADER_COLS.length + 2} className="text-center py-16 text-gray-400">加载中...</td></tr>
            ) : items.length === 0 ? (
              <tr><td colSpan={HEADER_COLS.length + 2} className="text-center py-16 text-gray-400">暂无数据</td></tr>
            ) : items.map(item => {
              const isExpanded = expandedId === item.id;
              return (
                <tr key={item.id} className="contents">
                  <td colSpan={HEADER_COLS.length + 2} className="p-0">
                    <div
                    ...
                    >
                      <div className="w-[40px] min-w-[40px] flex items-center justify-center px-2 py-2.5 shrink-0">
                        {isExpanded
                          ? <ChevronDown className="w-4 h-4 text-primary" />
                          : <ChevronRightIcon className="w-4 h-4 text-gray-400" />}
                      </div>
                      {HEADER_COLS.map(c => (
                        <div key={c.key} className="px-3 py-2.5 text-gray-700 dark:text-gray-300 whitespace-nowrap shrink-0"
                          style={{ width: c.width, minWidth: c.width }}>
                      ...
                      <div className="w-[100px] min-w-[100px] px-3 py-2.5 text-center shrink-0" onClick={e => e.stopPropagation()}>
                        <button onClick={() => { setEditData({ ...item }); setEditOpen(true); }}
                          className="text-primary hover:text-primary/80 text-xs"><Pencil className="w-3.5 h-3.5 inline mr-0.5" />编辑</button>
                      </div>
                    </div>
                    {isExpanded && <LineItems headerId={item.id} />}
                  </td>
                </tr>
              );
            })}
