...
const STAT_ROW1: StatItem[] = [
  { key: null,          label: '总任务数', numColor: 'text-navy-600 dark:text-cyan-400',   bg: 'bg-navy-50 dark:bg-white/[0.03]',     border: 'border-navy-100 dark:border-white/[0.06]',   activeBorder: 'ring-cyan-400/60' },
          ...
          <div>
            ...
            <div className="flex items-center">
              ...
              <div className="flex-1 relative h-1.5 bg-gray-200 dark:bg-white/[0.06] rounded-full overflow-visible mx-1">
                <div
                  className="absolute top-0 left-0 h-full bg-gradient-to-r from-cyan-500 to-cyan-400 rounded-full transition-all duration-500"
                  style={{ width: `${Math.max(progress, 2)}%` }}
                />
                <div
                  className="absolute top-1/2 -translate-y-1/2 w-2.5 h-2.5 bg-cyan-400 rounded-full border-2 border-white dark:border-white/[0.06] shadow-sm transition-all duration-500"
                  style={{ left: `${Math.max(progress, 1)}%` }}
                />
                <div
                  className="absolute top-full mt-0.5 text-[10px] text-cyan-400 font-medium -translate-x-1/2 transition-all duration-500"
                  style={{ left: `${Math.max(progress, 1)}%` }}
                >
                  {progress}%
                </div>
