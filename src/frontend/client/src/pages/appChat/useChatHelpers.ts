...
import { mepConfState, chatIdState, chatsState, currentChatState, currentRunningState, runningState } from "./store/atoms"
...
export default function useChatHelpers() {
    ...
    const wsUrl = useMemo(() => {
        ...
        const host = mepConfig?.websocket_url || window.location.host;
    ...
    const message = {
        ...
        createParsingMsg: (chatid: string) => {
            setChats((prev) =>
                updateChatMessages(prev, chatid, (messages) => [
                    ...messages,
                    {
