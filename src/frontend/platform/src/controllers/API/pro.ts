...
export const saveGroupApi = async (data: any): Promise<any> => {
    ...
    return await axios.post(`/api/v1/group/save`, {
        id,
        group_limit,
        admin_user,
        admin_user_id,
...
// GET sso URL – use native fetch to avoid global axios error toast when endpoint is unavailable
export function getSSOurlApi() {
    const baseURL = import.meta.env.BASE_URL || '';
    return fetch(`${baseURL}/api/oauth2/list`, { credentials: 'include' })
        .then(res => {
            if (!res.ok) return {};
