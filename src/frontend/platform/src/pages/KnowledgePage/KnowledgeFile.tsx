...
import { BookIcon } from "@/components/mep-icons/knowledge";
...
import { toast, useToast } from "@/components/mep-ui/toast/use-toast";
...
function CreateModal({ datalist, open, onOpenChange, onLoadEnd, mode = 'create', currentLib = null }) {
    ...
    const handleCreate = async (e, isImport = false) => {
        ...
        if (!name) {
            handleError(t('lib.enterLibraryName', { ns: 'mep' }));
            return;
        }
        if (name.length > 200) {
        ...
        if (!nameUnchanged && datalist.find(data => data.name === name && (!currentLib || data.id !== currentLib.id))) {
            handleError(t('lib.nameExists', { ns: 'mep' }));
            return;
        }

        if (descRef.current.value && desc.length > 200) {
            handleError(t('lib.descriptionLimit', { ns: 'mep' }));
            return;
        }
    ...
    return <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[625px]">
            <DialogHeader>
                <DialogTitle>{mode === 'create' ? t('lib.createLibrary', { ns: 'mep' }) : t('knowledgeBaseSettings')}</DialogTitle>
            </DialogHeader>
            <div className="flex flex-col gap-4 py-2">
                ...
                <div className="">
                    <label htmlFor="desc" className="mep-label">{t('lib.desc', { ns: 'mep' })}</label>
                    <Textarea
                        id="desc"
                        ref={descRef}
