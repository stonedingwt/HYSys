import { QaIcon } from "@/components/mep-icons/knowledge";
...
import { Dialog, DialogClose, DialogContent, DialogFooter, DialogHeader, DialogTitle } from "@/components/mep-ui/dialog";
import { Input, SearchInput, Textarea } from "@/components/mep-ui/input";
import AutoPagination from "@/components/mep-ui/pagination/autoPagination";
import { Select, SelectContent, SelectItem, SelectTrigger } from "@/components/mep-ui/select";
...
function CreateModal({ datalist, open, onOpenChange, onLoadEnd, mode = 'create', currentLib = null }) {
    ...
    const handleCreate = async (e, isImport = false) => {
        ...
        if (!name) {
            toast({ variant: 'error', description: t('lib.enterLibraryName', { ns: 'mep' }) });
            return;
        }
        if (name.length > 200) {
        ...
        if (descRef.current.value && desc.length > 200) {
            toast({ variant: 'error', description: t('lib.descriptionLimit', { ns: 'mep' }) });
            return;
        }
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[625px]">
                <DialogHeader>
                    <DialogTitle>{mode === 'create' ? t('lib.createLibrary', { ns: 'mep' }) : t('knowledgeSettings', { ns: 'mep' })}</DialogTitle>
                </DialogHeader>
                <div className="flex flex-col gap-4 py-2">
                    {mode === 'edit' && currentLib && (
                        <div className="space-y-4">
                            <div className="flex items-center gap-48">
                                <label className="mep-label text-sm text-gray-500">{t('lib.knowledgeBaseId', { ns: 'mep' })}</label>
                                <div className="text-sm">{currentLib.id}</div>
                            </div>
                            <div className="flex items-center gap-48">
                                <label className="mep-label text-sm text-gray-500">{t('createTime', { ns: 'mep' })}</label>
                                <div className="text-sm">{currentLib.create_time.replace('T', ' ')}</div>
                            </div>
                        </div>
                    )}
                    <div className="">
                        <label htmlFor="name" className="mep-label">{t('system.libraryName', { ns: 'mep' })}</label>
                        <span className="text-red-500">*</span>
                        <Input
                            name="name"
                            ref={nameRef}
                            placeholder={t('lib.enterLibraryName', { ns: 'mep' })}
                            className="col-span-3"
                        />
                    </div>
                    <div className="">
                        <label htmlFor="desc" className="mep-label">{t('knowledgeDesc')}</label>
                        <Textarea
                            id="desc"
                            ref={descRef}
                    ...
                    <div className="">
                        <label htmlFor="model" className="mep-label">{t('embeddingModel')}</label>
                        {isLoading ? (
...
export default function KnowledgeQa(params) {
    ...
    const handleDelete = (id) => {
        bsConfirm({
            title: t('prompt'),
            desc: t('lib.confirmDeleteLibrary', { ns: 'mep' }),
            onOk(next) {
        ...
        <div className="relative">
            ...
            <div className="h-[calc(100vh-128px)] overflow-y-auto pb-20">
                <div className="flex justify-end gap-4 items-center absolute right-0 top-[-44px]">
                    <SearchInput placeholder={t('lib.searchPlaceholder', { ns: 'mep' })} onChange={(e) => search(e.target.value)} />
                    <Button className="px-8 text-[#FFFFFF]" onClick={() => setOpen(true)}>{t('create', { ns: 'mep' })}</Button>
                </div>
                <Table>
                    <TableHeader>
                        <TableRow>
                            <TableHead>{t('lib.libraryName', { ns: 'mep' })}</TableHead>
                            <TableHead>{t('updateTime')}</TableHead>
                            <TableHead>{t('lib.createUser', { ns: 'mep' })}</TableHead>
            ...
            <div className="mep-table-footer px-6 bg-background-login">
                <p className="desc">{t('lib.libraryCollection', { ns: 'mep' })}</p>
                <div>
                    <AutoPagination
                        page={page}
                        pageSize={pageSize}
