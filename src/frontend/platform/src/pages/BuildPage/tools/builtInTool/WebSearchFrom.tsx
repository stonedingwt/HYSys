...
const WebSearchForm = ({ formData, onSubmit, isApi = false }: WebSearchFormProps) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const typedConfig = {
            qwen: (allToolsConfig as any).qwen || defaultToolParams.qwen,
            ...
            case 'qwen':
                return (
                    <p className="text-sm text-muted-foreground py-2">
                        使用通义千问模型的内置联网搜索能力，无需额外配置API Key。模型将在需要时自动搜索互联网获取最新信息。
                    </p>
                );
            case 'bing':
                return (
        ...
        <>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <div className="space-y-4">
                    <Label className="mep-label">{t('chatConfig.webSearch.config')}</Label>
                    {renderParams()}
                </div>
