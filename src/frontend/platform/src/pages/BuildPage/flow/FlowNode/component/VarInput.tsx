import { RbDragIcon } from "@/components/mep-icons/rbDrag";
import { Button } from "@/components/mep-ui/button";
import { Dialog, DialogContent, DialogTrigger } from "@/components/mep-ui/dialog";
        ...
        >
            {mini ?
                ...
                : <div className="flex justify-between gap-1 border-b dark:border-gray-600 px-2 py-1" onClick={() => textareaRef.current.focus()}>
                    <Label className="mep-label text-xs" onClick={validateVarAvailble}>
            ...
            <div
                ...
                className={`${heightClass} nowheel mep-richtext px-3 py-2 cursor-text whitespace-pre-line overflow-y-auto overflow-x-hidden border-none outline-none bg-search-input rounded-md dark:text-gray-50 placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50`}
            ></div>
            {children}
            {/* drag size */}
            <div
