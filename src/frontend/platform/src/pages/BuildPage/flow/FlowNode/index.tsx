...
import { Card, CardContent } from '@/components/mep-ui/card';
    ...
    if (isLeft) {
        return <div className={cname('absolute top-[58px] -left-[16px]', className)}>
            <Handle
                ...
                className='mep-flow-handle group'
                onClick={(e) => {
...
function CustomNode({ data: node, selected, isConnectable }: { data: WorkflowNode, selected: boolean, isConnectable: boolean }) {
    ...
    return (
        <div
            ...
            <div
                className={cname(`mep-node hover:border-primary/10 max-w-[35rem] ${nodeWidthClass} ${selected ? 'border-primary/10' : ' border-transparent'}`, nodeError && 'border-red-500')}
                data-id={node.id}
            >
                {/* top */}
                <RunLog node={node}>
                    <div className='mep-node-top flex items-center'>
                        {t('mep', { ns: 'mep' }) === 'MEP' && <LoadingIcon className='size-5 text-[#B3BBCD]' />}
                        <span className='node-face text-sm text-[#B3BBCD]'>{t('mep', { ns: 'mep' })}</span>
                    </div>
                </RunLog>

                {/* head */}
                <div className='mep-node-head'>
                    <div className='relative z-10 flex gap-2'>
                        ...
                        {!['output', 'condition', 'end'].includes(node.type) && <div
                            ...
                            <ChevronDown
                                className={`mep-label ${expend && 'rotate-180'}`}
                                size={16}
                            /></div>}
                    </div>
                    <EditText
