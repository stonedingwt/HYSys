...
import { DelIcon, LoadIcon } from "@/components/mep-icons";
...
import ActionButton from "@/components/mep-ui/button/actionButton";
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from "@/components/mep-ui/dialog";
import TextInput from "@/components/mep-ui/input/textInput";
...
const Header = ({ flow, nodes, onTabChange, preFlow, onPreFlowChange, onImportFlow }) => {
    ...
    const handleSaveNewVersion = async () => {
        ...
        message({
            variant: "success",
            title: `${t('skills.version', { ns: 'mep' })} v${maxNo} ${t('skills.saveSuccessful', { ns: 'mep' })}`,
            description: ""
        })
        // 更新版本列表
    ...
    return (
        <header className="flex justify-between items-center p-4 py-2 mep-bg border-b">
            {
            ...
            <div className="flex items-center gap-3">
                ...
                </Button>
                {
                    version && <ActionButton
                        ...
                        buttonTipContent={(
                            <div>
                                ...
                                <p className="mt-4 text-sm">{t('skills.supportVersions', { ns: 'mep' })}</p>
                            </div>
                        )}
                        dropDown={(
                    ...
                    >{t('skills.saveVersion', { ns: 'mep' })}</ActionButton>
                }
            ...
            <Dialog open={blocker.state === "blocked"}>
                <DialogContent className="sm:max-w-[425px]" close={false}>
                    <DialogHeader>
                        <DialogTitle>{t('prompt')}</DialogTitle>
                        <p className="mep-label pt-2">{isOnlineVersion ? t('onlineVersionMessage') : t('unsavedChangesMessage')}</p>
                    </DialogHeader>
                    <DialogFooter>
