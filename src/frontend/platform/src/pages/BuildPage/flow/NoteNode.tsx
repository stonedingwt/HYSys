...
function NoteNode({ data: node, selected, width, height }: { data: any; selected: boolean, width: number, height: number }) {
    ...
    return (
        <div
            className={`mep-node justify-start min-w-60 min-h-28 relative rounded-md border p-2 pt-4 bg-yellow-50 hover:border-orange-300 ${selected ? 'border-orange-300' : 'border-transparent'
                }`}
            data-id={node.id}
            style={{
