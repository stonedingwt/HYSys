        ...
        <DialogContent className="max-w-[820px]">
                ...
                <Table className="w-full">
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className="text-right">{t('mep:operations')}</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {services.map((el) => (
                            <TableRow key={el.id}>
                                ...
                                <TableCell className="py-2 text-right"><Button variant="link" className="h-8 rounded-full text-red-500 px-5" onClick={() => handleDel(el.id)}>{t('mep:delete')}</Button></TableCell>
                            </TableRow>
                        ))}
                        {showAdd && <TableRow>
                            ...
                            <TableCell>
                                <Button variant="link" className="h-8 rounded-full" onClick={handleAdd}>{t('mep:confirmButton')}</Button>
                                <Button variant="link" className="h-8 rounded-full text-gray-400" onClick={() => setShowAdd(false)}>{t('mep:cancel')}</Button>
                            </TableCell>
                        </TableRow>}
                    </TableBody>
                </Table>
            ...
            <DialogFooter>
                <div className="flex justify-start mt-4">
                    <Button variant='outline' className="flex w-[120px]" onClick={() => setShowAdd(true)}><Plus className="mr-2 size-5" />{t('mep:create')}</Button>
                </div>
            </DialogFooter>
        </DialogContent>
    </Dialog>
