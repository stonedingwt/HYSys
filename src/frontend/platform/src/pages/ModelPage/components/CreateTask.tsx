...
export default function CreateTask({ rtClick, gpuClick, onCancel, onCreate }) {
    ...
    return <div className="pt-2 h-[calc(100vh-162px)] px-2 overflow-y-auto">
        ...
        <div className="mt-6 flex gap-6">
            <Button disabled={loading} className="h-10 px-12" onClick={handleCreate}>{t('mep:create')}</Button>
            <Button disabled={loading} className="h-10 px-12" variant="outline" onClick={onCancel}>{t('mep:cancel')}</Button>
        </div>
    </div>
};
