...
import EditLabel from "../../../components/mep-ui/editLabel";
...
const HeadButtonView = ({ name, status, online, onPublish, onUnPublish, onDelete, onStop }) => {
    ...
    return <div className="absolute right-4 flex gap-2">
        ...
        {status === TaskStatus.TRAINING_IN_PROGRESS ?
            ...
            <Button size="sm" className="bg-red-400 hover:bg-red-500" onClick={deleteClick}>{t('mep:delete')}</Button>
        }
    </div>
}
...
export default function FinetuneDetail({ id, onDelete, onStatusChange }) {
    ...
    return <div>
        ...
        <div className="border-b pb-4">
            <div className="flex gap-4 mt-4">
                <small className="text-sm font-medium leading-none text-gray-500">{t('mep:createTime')}</small>
