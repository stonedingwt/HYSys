import { TrashIcon } from "@/components/mep-icons";
...
export default function AssisModel({ llmOptions, onBack }) {
    ...
    const handleSave = () => {
        ...
        if (form.llm_list.some(el => el.model_id === null)) {
            return message({ variant: 'error', description: t('model.assistantInferenceModel') + t('mep:required') })
        }
        if (form.auto_llm.model_id === null) {
            return message({ variant: 'error', description: t('model.assistantAutoOptimizationModel') + t('mep:required') })
        }
            ...
            <div className="mb-6">
                ...
                <div className="mt-2 border p-4 rounded-md bg-muted">
                    <div className="grid mb-4 items-center" style={{ gridTemplateColumns: "repeat(2, 1fr) 80px 110px 68px 90px 40px" }}>
                        <Label className="mep-label">{t('model.model')}<span className="text-red-500 text-xs">*</span></Label>
                        ...
                        <Label className="mep-label">{t('model.streamingOutput')}</Label>
                        <Label className="mep-label">
                        ...
                        <Label className="mep-label text-center">{t('model.setAsDefault')}</Label>
                        <div></div>
                    </div>
