...
export default function EvaluationModel({ llmOptions, onBack }) {
    ...
    const handleSave = () => {
        if (!selectedModel) {
            return message({ variant: 'error', description: t('model.defaultEvaluationFeature') + t('mep:required') })
        }
        const data = {
    ...
    return (
        <div className="max-w-[520px] mx-auto">
            <div className="mt-10">
                <Label className="mep-label">{t('model.defaultEvaluationFeature')}<span className="text-red-500 text-xs">*</span></Label>
                <ModelSelect
                    label={''}
                    value={selectedModel}
                    options={llmOptions}
