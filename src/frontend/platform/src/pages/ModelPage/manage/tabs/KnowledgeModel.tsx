...
import Cascader from "@/components/mep-ui/select/cascader";
...
const PromptDialog = ({ value, onChange, onRestore, onSave, children }) => {
    ...
    return <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent  className="sm:max-w-[625px] bg-background-login">
            ...
            <div>
                <Label className="mep-label">{t('model.docKnowledgeAbstractPrompt')}</Label>
                <Textarea
                    value={textValue}
                    onChange={(e) => {
...
export default function KnowledgeModel({ llmOptions, embeddings, onBack }) {
    ...
    const handleSave = async () => {
        ...
        const errors = []
        if (!embeddingModelId) {
            errors.push(t('model.defaultEmbeddingModel') + t('mep:required'))
        }
        if (!qaSimilarModelId) {
            errors.push(t('model.qaSimilarModel') + t('mep:required'))
        }
