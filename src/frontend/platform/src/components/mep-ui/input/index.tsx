...
import { SearchIcon } from "../../mep-icons/search"
        ...
        return (
            <div className={cname("relative w-full", boxClassName)}>
                <input
                    type={type}
                    className={cname(
                        "flex h-8 w-full rounded-lg border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-white/[0.05] px-3 py-1 text-sm text-slate-800 dark:text-slate-100 shadow-sm transition-all file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-slate-400 dark:placeholder:text-slate-500 focus-visible:outline-none focus-visible:border-slate-500 focus-visible:ring-2 focus-visible:ring-slate-500/20 dark:focus-visible:border-cyan-400/50 dark:focus-visible:ring-cyan-400/50 disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-slate-50 dark:disabled:bg-white/[0.03]",
...
export const PassInput = React.forwardRef<HTMLInputElement, InputProps & {
    ...
    return (
        <div className={cname("", className)}>
            <label htmlFor={id} className="mep-label flex items-center gap-1 mb-2">
                {label}
            ...
            {error && <p className="mep-tip mt-1">{typeof error === 'string' ? error : '不能为空'}</p>}
        </div>
    );
});
    ...
    ({ className, boxClassName = '', maxLength, value, defaultValue, onChange, ...props }, ref) => {
        ...
        return (
            <div className={cname('relative w-full', boxClassName)}>
                <textarea
                    className={cname(
                        "flex min-h-[80px] w-full rounded-lg border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-white/[0.05] px-3 py-2 text-sm text-slate-800 dark:text-slate-100 shadow-sm placeholder:text-slate-400 dark:placeholder:text-slate-500 focus-visible:outline-none focus-visible:border-slate-500 focus-visible:ring-2 focus-visible:ring-slate-500/20 dark:focus-visible:border-cyan-400/50 dark:focus-visible:ring-cyan-400/50 disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-slate-50 dark:disabled:bg-white/[0.03]",
                        className
                    )}
                    ref={ref}
                    defaultValue={defaultValue}
