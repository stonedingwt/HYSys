...
import { Card, CardContent, CardHeader, CardTitle } from '@/components/mep-ui/card';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/mep-ui/table';
        ...
        <section className='max-w-[1600px] flex-grow'>
            ...
            <Card className="mb-8">
                ...
                <CardContent>
                    ...
                    <p className='mep-label pb-2'>如时序图所示，在对接工作流 API 时，一般会经历以下步骤：</p>
                    <p className="mep-label pb-2"><span className="font-semibold">1. 第一步：</span>发起工作流执行。通过/invoke 接口让工作流从开始节点开始运行：</p>
                    ...
                    <p className="mep-label py-2"><span className="font-semibold">2. 第二步：</span>获取并解析工作流返回的事件。（一定要保留 session_id 等上下文信息，以便后续继续请求）</p>
                    <SyntaxHighlighter
        ...
        "events": [
            ...
            },
            {
                ...
                "input_schema": {
                    ...
                    <div className="mb-6">
                        <p className="mep-label py-2"><span className="font-semibold">3. 第三步：</span>根据事件类型渲染前端，并收集用户输入。</p>
                        <ul className="list-disc list-inside pl-4 mt-2 mep-label pb-2">
                    ...
                    <p className="mep-label py-2"><span className="font-semibold">4. 第四步：</span>带着用户输入再次调用 <code className="bg-gray-200 p-1 rounded">/invoke</code> 接口。</p>
                    <SyntaxHighlighter
                        className="w-full max-w-[80vw] overflow-auto custom-scroll text-sm"
                        language={'json'}
                        style={oneDark}
...
`}
                    ...
                    <p className="mep-label py-2"><span className="font-semibold">5. 第五步：</span>继续获取并解析返回的事件……直到返回 <code className="bg-gray-100 p-1 rounded">close</code> 事件（非必须）结束工作流运行，也可调用 <code className="bg-gray-100 p-1 rounded">POST /workflow/stop</code> 接口手动终止工作流。</p>
                </CardContent>
            </Card>
  ...
  "session_id": "string"
            ...
            <Card className="mb-8">
                ...
                <CardContent className='relative'>
                    <p className='mep-label py-2'>工作流返回的响应 JSON 通常包含一个或多个事件对象（数组形式），每个事件的字段可能不一样，以下枚举全部字段情况（非真实值，仅供参考）：</p>
                    <Table>
                        <TableHeader>
                            <TableRow>
  ...
  "output_schema": {# 需要在用户会话界面展示的数据
    ...
    "source_url": "",  # 知识库问答溯源页面地址，需要自己拼接元境的前端地址。具体是否支持溯源请参考产品文档
  ...
  "output_schema": {  # output
    "message": "本工作流可以解决xxxx等问题"
                    ...
                    <p className='mep-label mt-2'>处理逻辑：将 output_schema.message 展示给用户即可。</p>

                    <h3 className='mt-8' id="guide-2">引导问题事件</h3>
                    <p className='mep-label mt-2'>事件数据示例</p>
                    <Table>
                        <TableHeader>
                            <TableRow>
                                <TableHead className='w-[300px]'>样式预览</TableHead>
  ...
  "output_schema": {
    "message": [
      ...
      "引导问题2"
                    ...
                    <p className='mep-label mt-2'>处理逻辑：向用户展示引导问题列表，将用户选中的问题作为输入，继续调用工作流接口。</p>


                    <h3 className='mt-8' id="guide-3">等待输入事件-对话框形式</h3>
                    <div className='border border-red-200 rounded-sm bg-orange-100 p-4 text-sm'>
                        <p className='mep-label'>当工作流返回 <span className="bg-orange-50">event="input"</span> 且 <span className="bg-orange-50">input_type="dialog_input"</span>时，表示后端希望前端在对话框中接收用户输入以及上传文件（非必须）。</p>
                        <p className='mep-label mt-2'>下一次请求 <span className="bg-orange-50">/invoke</span> 接口必带的关键字段是 <span className="bg-orange-50">node_id</span>,<span className="bg-orange-50">message_id</span>,<span className="bg-orange-50">session_id</span> 以及对话框输入。</p>
                    </div>
                    <p className='mep-label mt-2'>事件数据示例</p>
                    <Table>
                        <TableHeader>
        ...
        "value": [
            ...
            },
            {
                ...
                "value": "all"  # 允许的文件类型
                    ...
                    <div className="mb-6">
                        <p className="mep-label py-2">处理逻辑：</p>
                        <ul className="list-disc list-inside pl-4 mt-2 mep-label pb-2">
            ...
            "user_input": "你好"
                    ...
                    <div className="mb-6">
                        <ul className="list-disc list-inside pl-4 mt-2 mep-label pb-2">
                            ...
                            <ul className="list-disc list-inside pl-4 mt-2 mep-label pb-2">
                                <li className='mt-2 leading-6'>如果有文件类型，调用元境文件上传接口获取到文件url，示例如下：</li>
                            </ul>
                        </ul>
                    </div>
                    <SyntaxHighlighter
...
`}
                    ...
                    <div className='border border-red-200 rounded-sm bg-orange-100 p-4 text-sm'>
                        <p className='mep-label'>当工作流返回 <span className="bg-orange-50">event="input"</span> 且 <span className="bg-orange-50">input_type="form_input"</span>时，后端希望前端渲染一个表单，让用户填写内容。</p>
                        <p className='mep-label mt-2'>下一次请求 <span className="bg-orange-50">/invoke</span> 接口必带的字段是 <span className="bg-orange-50">node_id</span>, <span className="bg-orange-50">message_id</span>, <span className="bg-orange-50">session_id</span> 以及用户填写的表单值。</p>
                    </div>
                    <p className='mep-label mt-2'>事件数据示例</p>
                    <Table>
                        <TableHeader>
...
def upload_file(local_path: str):
 ...
 financeB = upload_file("caibao2.pdf")`}
                    ...
                    <p className='mt-4 mep-label'>3. 提交时，JSON 中要和返回的 <code className="bg-gray-200 p-1 rounded">key</code> 对应，并带上 <code className="bg-gray-200 p-1 rounded">session_id</code>、<code className="bg-gray-200 p-1 rounded">message_id</code>、<code className="bg-gray-200 p-1 rounded">node_id</code> 等必备信息。</p>
                    <SyntaxHighlighter
                        className="w-full max-w-[80vw] overflow-auto custom-scroll"
                        language={'json'}
                        style={oneDark}
    ...
    "input": {
        "input_xxx": {  # 事件里的 node_id
            ...
            "category": "选项2" # 将选项内容赋值给变量。当允许多选时，多个选项内容通过逗号分隔。
                    ...
                    <p className="mep-label py-2">事件数据示例</p>
                    <Table>
                        <TableHeader>
                            <TableRow>
                                <TableHead className='w-[300px]'>样式预览</TableHead>
    ...
    "source_url": ""
                    ...
                    <div className="mb-6">
                        ...
                        <ul className="list-disc list-inside pl-4 mt-2 mep-label pb-2">
                            ...
                            <li className='mt-2 leading-6'><code className="bg-gray-200 p-1 rounded">source_url</code> 基于元境服务根路径，需要拼接元境访问地址才可访问</li>
                        </ul>
                    </div>
                    ...
                    <div className='border border-red-200 rounded-sm bg-orange-100 p-4 text-sm'>
                        <p className='mep-label'>此时工作流处于待输入状态</p>
                    </div>
                    <p className='mep-label mt-2'>事件数据示例</p>
                    <Table>
                        <TableHeader>
  ...
  "output_schema": {
    ...
    "output_key": "output_1" # output_1 是节点中的输出变量名
                    ...
                    <div className="mb-6">
                        <p className="mep-label py-2">调用方处理示例</p>
                        <ul className="list-disc list-inside pl-4 mt-2 mep-label pb-2">
  ...
  "output_schema": {
    ...
    "source_url": "" # 是否支持溯源请参考产品文档
                    ...
                    <div className='border border-red-200 rounded-sm bg-orange-100 p-4 text-sm'>
                        <p className='mep-label'>当接收到 <span className="bg-orange-50">event="close"</span> 时，代表工作流已运行结束</p>
                    </div>
                    <p className='mep-label mt-2'>事件数据示例</p>
                    <SyntaxHighlighter
                        className="w-full overflow-auto custom-scroll"
